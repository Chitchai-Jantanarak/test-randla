...
# ---------------------------------------------------------------------------
# Extract per-building LAS files from JSON bounding boxes
# ---------------------------------------------------------------------------

def extract_buildings_las(
    input_path: str,
    ...
    """Extract individual LAS files per building using JSON bounding boxes.

    For each building in the JSON, crops the source point cloud to that
    building's 3D bounding box (+ optional padding) and writes a separate
    LAS file.

    Args:
        input_path: Source point cloud (.las/.laz/.ply).
        buildings_json_path: JSON file from apply_labels (contains bbox_min/bbox_max).
        output_dir: Directory to write per-building LAS files.
        padding: Extra meters around each bbox (catches edge points).
        use_cluster_labels: If True, include only points classified as building
            (ASPRS 6) within the bbox. If False, include all points in the bbox.
    """
    ...
    # if source is classified LAS, use its classification to filter building pts
    src_classification = None
    if source_las is not None:
        try:
            src_classification = np.array(source_las.classification)
        except Exception:
            pass
    ...
    for b in buildings:
        bid = b["id"]
        bmin = np.array(b["bbox_min"]) - padding
        bmax = np.array(b["bbox_max"]) + padding
        ...
        # optionally restrict to building-classified points only
        if use_cluster_labels and src_classification is not None:
            mask = mask & (src_classification == BUILDING_ASPRS_CLASS)

        if mask.sum() == 0:
            print(f"  Building {bid}: no points in bbox — skipped")
            continue

        bld_points = points[mask]
        bld_colors = colors[mask] if colors.size > 0 and colors.ndim == 2 else np.empty((0,))

        out_path = os.path.join(output_dir, f"building_{bid:04d}.las")
        cls_arr = np.full(len(bld_points), BUILDING_ASPRS_CLASS, dtype=np.uint8)
        write_classified_las(out_path, bld_points, bld_colors, cls_arr)

        w = b["bbox_max"][0] - b["bbox_min"][0]
        h = b["bbox_max"][1] - b["bbox_min"][1]
        z = b["bbox_max"][2] - b["bbox_min"][2]
        print(f"  Building {bid}: {len(bld_points):,} pts → {out_path}  "
              f"({w:.1f}x{h:.1f}x{z:.1f}m)")
        extracted += 1

    print(f"\nExtracted {extracted}/{len(buildings)} buildings to {output_dir}/")


def extract_buildings_las_chunked(
    input_path: str,
    buildings_json_path: str,
    output_dir: str,
    padding: float = 0.5,
    ...
    """Memory-efficient per-building LAS extraction for large files.

    Streams through the source file in chunks, collecting points that fall
    within each building's padded bbox, then writes one LAS per building.
    """
    with open(buildings_json_path) as f:
        report = json.load(f)
    ...
    # precompute padded bboxes
    bboxes_min = np.array([b["bbox_min"] for b in buildings]) - padding
    bboxes_max = np.array([b["bbox_max"] for b in buildings]) + padding
    n_buildings = len(buildings)

    # accumulators: list of point arrays per building
    building_points = [[] for _ in range(n_buildings)]
    building_colors = [[] for _ in range(n_buildings)]

    for pts, cols, chunk_idx in read_point_cloud_chunked(input_path, chunk_size):
        for i in range(n_buildings):
            mask = (
                (pts[:, 0] >= bboxes_min[i, 0]) & (pts[:, 0] <= bboxes_max[i, 0]) &
                (pts[:, 1] >= bboxes_min[i, 1]) & (pts[:, 1] <= bboxes_max[i, 1]) &
                (pts[:, 2] >= bboxes_min[i, 2]) & (pts[:, 2] <= bboxes_max[i, 2])
            )
            if mask.any():
                building_points[i].append(pts[mask])
                if cols.size > 0 and cols.ndim == 2:
                    building_colors[i].append(cols[mask])

        print(f"  chunk {chunk_idx}: scanned against {n_buildings} bboxes")

    extracted = 0
    for i, b in enumerate(buildings):
        if not building_points[i]:
            print(f"  Building {b['id']}: no points — skipped")
            continue

        bld_pts = np.concatenate(building_points[i])
        bld_cols = np.concatenate(building_colors[i]) if building_colors[i] else np.empty((0,))

        out_path = os.path.join(output_dir, f"building_{b['id']:04d}.las")
        cls_arr = np.full(len(bld_pts), BUILDING_ASPRS_CLASS, dtype=np.uint8)
        write_classified_las(out_path, bld_pts, bld_cols, cls_arr)

        w = b["bbox_max"][0] - b["bbox_min"][0]
        h = b["bbox_max"][1] - b["bbox_min"][1]
        z = b["bbox_max"][2] - b["bbox_min"][2]
        print(f"  Building {b['id']}: {len(bld_pts):,} pts → {out_path}  "
              f"({w:.1f}x{h:.1f}x{z:.1f}m)")
        extracted += 1

    print(f"\nExtracted {extracted}/{n_buildings} buildings to {output_dir}/")
...
if __name__ == "__main__":
    ...
    # per-building LAS extraction
    parser.add_argument("--extract-buildings-dir", default=None,
                        help="Output dir for per-building LAS files (auto-extracts after counting)")
    parser.add_argument("--extract-from-json", default=None,
                        help="Standalone: extract building LAS files from existing JSON + source")
    parser.add_argument("--bbox-padding", type=float, default=0.5,
                        help="Padding (meters) around each building bbox when extracting")
    parser.add_argument("--all-points-in-bbox", action="store_true",
                        help="Include ALL points in bbox, not just building-classified ones")

    args = parser.parse_args()

    # --- standalone extraction mode: JSON + source → per-building LAS ---
    if args.extract_from_json:
        if args.chunk_size > 0:
            extract_buildings_las_chunked(
                input_path=args.input,
                buildings_json_path=args.extract_from_json,
                output_dir=args.extract_buildings_dir or "buildings_extracted",
                padding=args.bbox_padding,
                chunk_size=args.chunk_size,
            )
        else:
            extract_buildings_las(
                input_path=args.input,
                buildings_json_path=args.extract_from_json,
                output_dir=args.extract_buildings_dir or "buildings_extracted",
                padding=args.bbox_padding,
                use_cluster_labels=not args.all_points_in_bbox,
            )
        sys.exit(0)
    ...
    # auto-extract per-building LAS if requested
    if args.extract_buildings_dir:
        json_path = args.buildings_json or os.path.splitext(args.output)[0] + "_buildings.json"
        if os.path.exists(json_path):
            print(f"\n--- Extracting per-building LAS files ---")
            if args.chunk_size > 0:
                extract_buildings_las_chunked(
                    input_path=args.input,
                    buildings_json_path=json_path,
                    output_dir=args.extract_buildings_dir,
                    padding=args.bbox_padding,
                    chunk_size=args.chunk_size,
                )
            else:
